...
```env
# Flask
OPENAI_API_KEY=...              # Keep for now, may remove later
...
# Next.js
NEXT_PUBLIC_API_URL=http://flask:6000   # Internal Docker network
...
- Next.js talks to Flask over Docker's internal network (`http://flask:6000`)
- Browser talks to Next.js on port 3000
- iPhone Shortcuts talk to Flask on port 6000 directly
- CORS configured on Flask to allow requests from Next.js origin
...
CREATE TABLE IF NOT EXISTS ideas (
    ...
    content         TEXT,                        -- idea text, nullable (image-only ideas)
    timestamp       DATETIME NOT NULL,           -- Vancouver timezone
...
- Add `media_type`, `has_media`, `category_id` columns to `ideas` if missing
- Create `idea_media` and `categories` tables if missing
- Seed default categories if `categories` table is empty
- Backfill: set all existing ideas to `media_type='text'`, `has_media=0`
- Backfill: regenerate all embeddings with nomic model (one-time, can be a separate script)
- Backfill: auto-categorize all existing ideas
...
**Performance:** Single query with JOINs. No pagination — all ideas loaded at once (for a personal app, even thousands of ideas is small). Media URLs are relative paths served by Flask.
...
4. On save:
   ...
   - `POST /ideas` sends text + category
   - If media attached: `POST /upload` sends files in parallel
   - If either fails: toast error, remove optimistic entry
...
| Scroll performance | 60fps | Virtualized list if > 500 ideas (react-window) |
...
- **Image thumbnails:** For the ideas feed, serve compressed thumbnails. Full-res only on detail view. Flask can generate thumbnails on upload using Pillow, or we do it client-side.
...
- **Virtualized list:** If idea count exceeds ~500, switch to `react-window` for the feed to maintain scroll performance.
...
services:
  ...
  nextjs:
    ...
    ports:
      - "3000:3000"
    environment:
      - NEXT_PUBLIC_API_URL=http://flask:6000
...
3. Remove the old Streamlit service from docker-compose
4. Update any bookmarks/shortcuts from :8502 to :3000
5. Old `stl.py` can be deleted once confirmed
...
- **React components:** PascalCase files (`IdeaCard.tsx`), kebab-case directories (`idea-card/`)
  - Exception: shadcn components use their default kebab-case (`button.tsx`, `drawer.tsx`)
...
1. **Define the data** — if the feature needs new data:
   - Add the table/columns in `app.py`'s `init_db()` function (migration style: `ALTER TABLE ADD COLUMN IF NOT EXISTS`)
   - Add TypeScript types in `frontend/lib/types.ts`

2. **Add the API endpoint** — in `app.py`:
   - Follow existing patterns: validate input, use Vancouver timestamps, return JSON
   - If the endpoint does heavy work (ML, file processing), do it in a background thread and return immediately

3. **Add the API client** — in `frontend/lib/api.ts`:
   - Add a function that calls the new endpoint
...
- **Background processing for heavy work.** Embeddings, categorization, and thumbnail generation happen in background threads. Never block a request.
